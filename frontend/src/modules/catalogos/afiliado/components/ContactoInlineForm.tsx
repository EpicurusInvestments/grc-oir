/** Formulario INLINE de contacto (dentro del detalle o el propio alta/edición del
 * afiliado), mismo patrón que `ContactoInlineForm` de Anunciante/Agencia.
 *
 * La raíz es un `<div>`, NO un `<form>`: esta pieza se monta DENTRO del `<form>` de
 * alta/edición de Afiliado (`ContactosSection` dentro de `AfiliadoForm`) — un `<form>`
 * anidado dentro de otro no es HTML válido (el navegador lo aplana y el botón "Guardar"
 * terminaría disparando el `submit` del formulario EXTERIOR, bug real ya corregido una
 * vez en Anunciante/Agencia — ver ADR-092). `handleSubmit` de react-hook-form no
 * necesita un evento de `<form>`, se invoca directo desde el `onClick` del botón.
 */
