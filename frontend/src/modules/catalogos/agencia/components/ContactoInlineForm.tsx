/** Formulario INLINE de contacto (dentro del detalle de la agencia), mismo patrón que
 * `ContactoInlineForm` del módulo Anunciante. El agencia_id lo fija la pantalla.
 *
 * Fix: la raíz es un `<div>`, NO un `<form>`. Desde ADR-092 esta pieza también se monta
 * DENTRO del `<form>` de alta/edición de Agencia (`ContactosSection` dentro de
 * `AgenciaForm`) — un `<form>` anidado dentro de otro no es HTML válido: el navegador
 * lo aplana, así que el botón "Guardar" terminaba disparando el `submit` del formulario
 * EXTERIOR (el de la agencia), no el de este contacto. `handleSubmit` de react-hook-form
...
export function ContactoInlineForm({
  ...
  return (
    ...
    >
      ...
      <input className="fi" autoFocus placeholder="Ej. Carla Mora" {...register("nombre_contacto")} />
